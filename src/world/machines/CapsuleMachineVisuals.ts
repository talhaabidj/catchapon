...
const CLEAN_GLASS_COLOR = new THREE.Color(0xf0f8ff);
const DIRTY_GLASS_COLOR = new THREE.Color(0x3a3520);
...
export function syncMachineMaintenanceVisuals(
  ...
  if (visualRefs && visualRefs.length > 0) {
    ...
    const targetGlassOpacity = isDirty ? 0.7 : 0.25;
    const targetGlassRoughness = isDirty ? 0.8 : 0.05;
